...
class SettingsMenu(QWidget):
    ...
    def __init__(self):
        ...
        min_rate_limit = self.settings.get("minRateLimit", RequestHandler.DEFAULT_MIN_RATE_LIMIT)
        self.request_handler.update_min_rate_limit(min_rate_limit)
        self.logger.debug(f"Loaded min rate limit of {min_rate_limit}.")

        max_rate_limit = self.settings.get("maxRateLimit", RequestHandler.DEFAULT_MAX_RATE_LIMIT)
    ...
    def update_min_rate(self):
        ...
        if value < RequestHandler.ABS_MIN_RATE_LIMIT:
            self.logger.warning(f"Min rate limit must be greater than {RequestHandler.ABS_MIN_RATE_LIMIT}. Setting to {RequestHandler.ABS_MIN_RATE_LIMIT}.")
        ...
        if value != self.settings.get("minRateLimit", RequestHandler.DEFAULT_MIN_RATE_LIMIT):
    ...
    def update_max_rate(self):
        ...
        if value < self.ui.minRateBox.value():
            self.logger.warning("Max rate limit must be equal to or greater than min rate limit. Setting to min rate limit.")
