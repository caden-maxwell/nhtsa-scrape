...
class ScatterplotWorker(QRunnable):
    ...
    def run(self):
        ...
        # matplotlib.pyplot.plot(dfn.c_bar, fit_e[0] * dfn.c_bar + fit_e[1], color='red', linewidth=2)
        ...
        # matplotlib.pyplot.legend(labels=['NASS, ' + 'r$\mathregular{^2}$ value = ' + r2 + ' - ' + s_eq, 'Total, ' + 'r$\mathregular{^2}$ value = ' + r2_e + ' - ' + s_eq_e], fontsize=14)
        matplotlib.pyplot.legend(
            labels=["NASS, " + "r$\mathregular{^2}$ value = " + r2 + " - " + s_eq],
            fontsize=14,
        )
        matplotlib.pyplot.xlabel("Crush (inches)", size=24)
        matplotlib.pyplot.ylabel("Change in Velocity (mph)", size=24)
        for i, label in enumerate(data_frame["case_id"]):
            matplotlib.pyplot.text(data_frame.c_bar[i], data_frame.NASS_dv[i], label)
        # for i, label in enumerate(df.index):
        #    matplotlib.pyplot.text(dfn.c_bar[i], dfn.TOT_dv[i],label)

        os.makedirs(self.path, exist_ok=True)
        matplotlib.pyplot.savefig(self.path / "NASS_Analysis.png", dpi=150)
